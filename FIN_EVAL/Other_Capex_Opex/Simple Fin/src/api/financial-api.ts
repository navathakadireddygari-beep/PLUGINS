...
export function buildFinancialUrl(cfg: AppConfig): string {
  ...
  if (cfg.spc_type_id != null) params.set("spc_type_id", String(cfg.spc_type_id));
  if (cfg.template_type_id != null) params.set("template_type_id", String(cfg.template_type_id));
  const query = params.toString();
  if (query) {
    return `${base}?${query}`;
  }
  throw new Error("Missing proposal_id, spc_type_id, or template_type_id in window.__APP_CONFIG__");
}
...
export function buildFinancialPostUrl(cfg: AppConfig): string {
  if (!cfg.api_endpoint) throw new Error("Missing api_endpoint in window.__APP_CONFIG__");
  return `${cfg.api_endpoint}/GIS/proposalAuthoring/financialEvaluation`;
}
...
export function transformApiData(
  ...
  const isVisibleTemplateSection = (section: ApiSection): boolean => {
    const token = (section.section_type || section.section_name || "")
      ...
      .replace(/[^A-Z0-9]/g, "");
    return (
      token === "PLINVESTMENTOPEX" ||
      token === "PL" ||
      token === "RETURNSANALYSIS" ||
      token === "RETURNANALYSIS" ||
      token === "CASHFLOW"
    );
  };
  const visibleSections = sections.filter(isVisibleTemplateSection);
  ...
  const groups: Group[] = [...visibleSections]
...
export function buildPutPayload(
  ...
  const sections = tableData.groups.map((group, grpIdx) => {
    const lines = group.values.map((value, lineIdx) => {
      ...
      return {
        ...
        template_fin_eval_line_id: value.templateLineId ?? null,
        ...
        is_read_only:        value.isReadOnly     ?? "N",
        is_mandatory:        value.isMandatory    ?? "N",
        ...
        line_item_code:       value.lineItemCode ?? null,
      };
    });

    return {
      fin_eval_section_id: group.sectionId    ?? null,
      template_section_id: group.templateSectionId ?? null,
      section_name:        group.name,
      section_type:        group.sectionType  ?? "CUSTOM",
      is_custom:           group.isCustom     ?? "Y",
      is_read_only:        group.isReadOnly   ?? "N",
      is_account_required: group.isAccountRequired ?? "N",
      is_new_line_required: group.isNewLineRequired ?? "Y",
      is_mandatory:        group.isMandatory  ?? "N",
  ...
  return {
    ...
    proposal_id:               rawHeader.proposal_id ?? null,
    spc_type_id:               getAppConfigSafeNumber("spc_type_id"),
    template_type_id:          getAppConfigSafeNumber("template_type_id"),
    user_email:                appUser,
...
function getAppConfigSafeNumber(key: "spc_type_id" | "template_type_id"): number | null {
  if (typeof window === "undefined") return null;
  const cfg = (window as unknown as { __APP_CONFIG__?: Record<string, unknown> }).__APP_CONFIG__;
...
function pickExchangeRate(raw: ExchangeRateResponse): number | null {
  return (
    toPositiveNumber(raw.data?.usd_fbr) ??
    toPositiveNumber(raw.usd_fbr) ??
    toPositiveNumber(raw.exchange_rate) ??
    toPositiveNumber(raw.exchangeRate) ??
    toPositiveNumber(raw.rate) ??
    toPositiveNumber(raw.data?.exchange_rate) ??
    toPositiveNumber(raw.data?.exchangeRate) ??
    toPositiveNumber(raw.data?.rate) ??
    toPositiveNumber(raw.data?.items?.exchange_rate) ??
    toPositiveNumber(raw.data?.items?.exchangeRate) ??
    toPositiveNumber(raw.data?.items?.rate) ??
    null
  );
}

/**
 * Fetch exchange rate for a local currency.
 * POST {api_endpoint}/GIS/proposalAuthoring/currencyExchangeRates
 * body: { year_period: "202608", currency: "USD" | "CAD" | ... }
 */
export async function getCurrencyExchangeRate(
  ...
  const payload = { year_period: yearPeriod, currency: normalizedCurrency };
  ...
  const raw = (await res.json()) as ExchangeRateResponse;
  return pickExchangeRate(raw);
}
...
export async function saveFinancialData(
  ...
  const hasProposalId = Boolean(cfg.proposal_id ?? rawHeader.proposal_id);
  const method  = hasProposalId ? "PUT" : "POST";
  const url     = hasProposalId ? buildFinancialPutUrl(cfg, rawHeader) : buildFinancialPostUrl(cfg);
