...
// Decimal places shown per scale — K is whole numbers, M/B need finer precision.
...
export const formatNumber = (value: number, fmt: NumberFormatCode, decimals: number = 2): string => {
...
export const formatSignedAmount = (n: number, fmt: NumberFormatCode, decimals: number = 2): string => {
  if (!n) return "—";
...
export const formatDisplayValue = (
  ...
  return formatSignedAmount(v, opts.numberFormat, SCALE_DECIMALS[opts.scale]);
};
