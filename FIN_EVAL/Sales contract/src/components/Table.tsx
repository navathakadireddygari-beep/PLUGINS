...
// Statuses an author (non-read-only role) can edit; anything else is read-only.
const EDITABLE_PROPOSAL_STATUSES: ReadonlySet<string> = new Set(["DRAFT", "REJECTED", "NEEDS_REAPPROVAL"]);
...
export default function PivotTableWithAPI(): React.ReactElement {
  ...
  /* ── Access control ───────────────────────────────────────────────
     Role check resolved server-side (owner → collaborator → JSON role) and
     injected via the config global; getAppConfig() fails closed when absent.
     A read-only role always wins regardless of proposal_status. Otherwise,
     editability is gated by proposal_status: DRAFT/REJECTED/NEEDS_REAPPROVAL
     are editable for an author; SUBMITTED/IN_PROGRESS/APPROVED (or any
     status not yet loaded/recognized) are read-only. When read-only, every
     mutation handler short-circuits (which also freezes the controlled
     inputs) and the editing controls are not rendered.
     Exception: no proposal exists yet (proposal_id absent, spc_type_id +
     template_type_id present) — there is no proposal_status to gate on, so
     this "create new proposal" flow is fully editable regardless of status. */
  const isRoleReadOnly = useState<boolean>(() => getAppConfig().is_readonly)[0];
  const proposalStatus = (rawHeader?.proposal_status || "").toUpperCase();
  const cfg = getAppConfig();
  const isNewProposal = cfg.proposal_id == null && cfg.spc_type_id != null && cfg.template_type_id != null;
  const isReadOnly = isRoleReadOnly || (!isNewProposal && !EDITABLE_PROPOSAL_STATUSES.has(proposalStatus));
  ...
  useEffect(() => {
    ...
    setFxError(null);
    const cfg = getAppConfig();
    getCurrencyExchangeRate(cfg, localCurrency)
      .then((rate) => {
        if (cancelled) return;
        if (rate == null) setFxError("Exchange rate unavailable.");
        setFxRate(rate);
      })
      .catch((e: unknown) => {
        if (cancelled) return;
  ...
  const displayFmt = (n: number): string => {
    ...
    const formatted = formatNumber(Math.abs(v), numFmt, scaleDecimals(displayScale));
  ...
  const numCellFocus = (cellKey: string, storedVal: number): void => {
    ...
    setNumEdit({ key: cellKey, text: storedVal === 0 ? "" : plainNumber(dv, numFmt, scaleDecimals(displayScale)) });
  };
      ...
      <div ref={headerRef} style={{ position: headerPinned ? "fixed" : "sticky", top: 0, left: headerPinned ? leftOffset : undefined, width: headerPinned ? containerWidth : undefined, zIndex: 200, background: "#fff", borderBottom: "1px solid #e5e7eb" }}>
        ...
        <div className="action-bar">
          <div className="action-bar-group">
            ...
            {/* Export hidden for read-only users and non-editable proposal statuses */}
            {!isReadOnly && (
              <button
