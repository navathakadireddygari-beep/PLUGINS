 ...
 *   - file_id          : Staged import file id — required by the
 *                        finEvaluationStaging GET/PUT and finEvalStagingMigrate
 *                        POST endpoints
...
export function getAppConfig(): AppConfig {
  ...
  return {
    ...
    // Default to locked when the host page doesn't send an explicit value —
    // fail safe (read-only) rather than fail open (editable).
    is_readonly:      cfg.is_readonly !== false,
