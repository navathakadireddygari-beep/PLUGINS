...
import {
  SCALE_OPTIONS, SCALE_DIVISOR, SCALE_TOOLTIP, decimalsForScale, type ScaleCode,
...
const stripSelect: React.CSSProperties = {
  border: "1px solid #e5e7eb", borderRadius: 6, padding: "5px 10px",
  fontSize: 12, fontWeight: 700, color: "#374151", backgroundColor: "#fff",
  cursor: "pointer",
};

const stripLabel: React.CSSProperties = { fontSize: 11, color: "#6b7280", fontWeight: 700, letterSpacing: 0.5 };
...
export default function PivotTableWithAPI(): React.ReactElement {
  ...
  // Fetch the live local→USD rate only when a conversion is actually in
  // effect (display currency differs from local currency) — when they
  // match (e.g. both USD) there's nothing to convert, so the FX strip stays hidden.
  const localCurrency  = (rawHeader?.local_currency   || "USD").toUpperCase();
  const displayCurrency = (rawHeader?.display_currency || localCurrency).toUpperCase();
  useEffect(() => {
    if (!rawHeader || localCurrency === displayCurrency) { setLiveFxRate(null); return; }
    const cfg = getAppConfig();
    const now = new Date();
    const yearPeriod = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, "0")}`;
    getCurrencyExchangeRate(cfg, yearPeriod, localCurrency)
      .then(setLiveFxRate)
      .catch((e) => console.error("[currency-exchange-rate] failed", e));
  }, [rawHeader, localCurrency, displayCurrency]);
  ...
  // Scroll a highlighted line row into view, then auto-clear the highlight.
  useEffect(() => {
    ...
    const t = setTimeout(() => setHighlightLineId(null), 3000);
    return () => clearTimeout(t);
  }, [highlightLineId, data]);

  // Scroll a highlighted header/KPI field into view, then auto-clear the highlight.
  useEffect(() => {
    ...
    const t = setTimeout(() => setHighlightHeaderField(null), 3000);
    return () => clearTimeout(t);
  }, [highlightHeaderField]);

  // Auto-clear a highlighted section header (scrolling there is already
  // handled by the scrollTarget effect above).
  useEffect(() => {
    if (!highlightSectionId) return;
    const t = setTimeout(() => setHighlightSectionId(null), 3000);
    return () => clearTimeout(t);
  }, [highlightSectionId]);

  // Navigate to the exact place a validation issue is coming from: a header
  // field, a section (expand + scroll), or a specific line (expand its
  // parent section, then scroll/highlight the line row itself).
  const goToError = (issue: ValidationIssue): void => {
    if (issue.headerField) {
      setHighlightHeaderField(issue.headerField);
    } else if (issue.sectionId != null) {
      setData((p) => p ? ({ ...p, groups: p.groups.map((g) => g.sectionId === issue.sectionId ? { ...g, expanded: true } : g) }) : p);
      setScrollTarget(String(issue.sectionId));
      setHighlightSectionId(String(issue.sectionId));
    } else if (issue.lineId) {
      const targetLineId = issue.lineId;
      setData((p) => {
        if (!p) return p;
        const owner = p.groups.find((g) => g.values.some((v) => v.id === targetLineId));
        if (!owner) return p;
        return { ...p, groups: p.groups.map((g) => g.id === owner.id ? { ...g, expanded: true } : g) };
      });
  ...
  // After a successful save/validate the backend holds the source of truth
  // and returns every section and line (calculated/template rows included,
  // even empty ones), so we adopt it wholesale — exactly what a page reload
  // does. We only carry over each section's expanded/collapsed state so the
  // view doesn't jump.
  const silentRefetch = useCallback(async (label: string): Promise<void> => {
    const cfg = getAppConfig();
    try {
      ...
      setValidationErrors(refetched.validationErrors);
      setErrorsDismissed(false);
    } catch (e) {
  ...
  /* ── Validate ─────────────────────────────────────────────────
     PUT the current state to the import/validate endpoint, then
     silently re-fetch so the view reflects the validated state. */
  const handleValidate = useCallback(async (): Promise<void> => {
    ...
    setSaveError(null);
    try {
      const response = await validateFinancialData(cfg, data, rawHeader);
      setToast({
        kind: "success",
        message: ((response as any).apiMessage || response.api_message || "") || "Financial evaluation validated successfully.",
      });
      await silentRefetch("validate");
    } catch (e: unknown) {
      clearCachedToken();
  ...
  }, [data, rawHeader, silentRefetch]);

  /* ── Save Model ───────────────────────────────────────────────
     POST to finEvalStagingMigrate, then silently re-fetch. */
  const saveDraft = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      setToast({
        kind: "success",
        message: ((response as any).apiMessage || response.api_message || "") || "Financial evaluation saved successfully.",
      });
      const saveEvent = new CustomEvent("tool:fin_eval_saved", { detail: { proposalId } });
      console.log("[fin_eval] dispatching tool:fin_eval_saved", saveEvent.detail);
      window.dispatchEvent(saveEvent);
      publishToBridge({ type: "PLUGIN_MSG", source: "editor", target: "chatbot", action: "fin-eval:saved", payload: { proposalId, method: "POST" } });
      await silentRefetch("saveDraft");
    } catch (e: unknown) {
      clearCachedToken();
  ...
  const rawFmt = (n: number): string => formatSignedAmount(n, numberFormat, decimalsForScale(displayScale));
    ...
    <div ref={containerRef}>
      ...
      <div ref={headerRef} style={{ position: headerPinned ? "fixed" : "sticky", top: 0, left: headerPinned ? leftOffset : undefined, width: headerPinned ? containerWidth : undefined, zIndex: 200, background: "#fff", borderBottom: "1px solid #e5e7eb" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", padding: "10px 0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 18px", border: "1px solid #00A000", borderRadius: 6, fontSize: 14, fontWeight: 700, background: "#fff", color: "#00A000", cursor: validating || loading ? "not-allowed" : "pointer", opacity: validating || loading ? 0.7 : 1 }}
            >
              <Check size={14} style={validating ? { animation: "spin 1s linear infinite" } : {}} />
            ...
            <button
              ...
              type="button"
              onClick={saveDraft}
              disabled={savingDraft || loading || validationErrors.length > 0}
              style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 24px", border: "none", borderRadius: 6, fontSize: 14, fontWeight: 700, background: BRAND, color: "#fff", cursor: savingDraft || loading || validationErrors.length > 0 ? "not-allowed" : "pointer", opacity: savingDraft || loading || validationErrors.length > 0 ? 0.7 : 1 }}
            >
        ...
        {rawHeader && (() => {
          ...
          const showFxRate = local !== display && liveFxRate != null;
          const fxRate     = liveFxRate ?? 0;
            ...
            >
              ...
              {stripTooltip === kind && stripTooltipPos && createPortal(
                ...
                <span
                  style={{
                    position: "fixed",
                    left: align === "right" ? stripTooltipPos.left - 300 - 6 : stripTooltipPos.right + 6,
                    top: stripTooltipPos.top - 4,
                    background: DARK_TOGGLE, color: "#fff", padding: "6px 10px", borderRadius: 4, fontSize: 13, whiteSpace: "normal", width: 300, lineHeight: 1.5, zIndex: 9999, boxShadow: "0 4px 14px rgba(0,0,0,.25)", pointerEvents: "none",
                  }}
                >
                  {tooltipText}
                </span>,
          ...
          return (
            <div className="feval-strip" style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, padding: "10px 16px", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
              <span style={stripLabel}>CURRENCY</span>
              <div data-kpi-field="display_currency" style={{ display: "flex", border: highlightHeaderField === "display_currency" ? "2px solid #f59e0b" : "1px solid #e5e7eb", borderRadius: 6, overflow: "hidden", boxShadow: highlightHeaderField === "display_currency" ? "0 0 0 3px #fde68a" : "none" }}>
              ...
              <div style={{ display: "flex", border: "1px solid #e5e7eb", borderRadius: 6, overflow: "hidden" }}>
                {SCALE_OPTIONS.map((s, i) => (
                  <button key={s.code} type="button" onClick={() => handleScaleChange(s.code)} style={toggleBtn(displayScale === s.code, i === 0)}>
                    {s.label}
                  </button>
                ))}
              </div>
              ...
              <span style={stripLabel}>NUMBER</span>
              <select
                value={numberFormat}
              ...
              <span style={stripLabel}>DATE</span>
              <select
                value={dateFormat}
              ...
              {/* FX rate indicator — only when a conversion is actually in effect */}
              {showFxRate && (
                <div style={{ marginLeft: "auto", display: "flex", flexDirection: "column", alignItems: "flex-end", fontSize: 11, color: "#374151", lineHeight: 1.5 }}>
                  <span>1 {local} = {fxRate.toFixed(4)} USD</span>
                  <span>1 USD = {(fxRate ? 1 / fxRate : 0).toFixed(4)} {local}</span>
                </div>
              )}
            </div>
          );
      ...
      {validationErrors.length > 0 && !errorsDismissed && (
        <div style={{ border: "1px solid #fecaca", background: "#fef2f2", borderRadius: 8, marginTop: 16, overflow: "hidden" }}>
          ...
          <div style={{ maxHeight: 220, overflowY: "auto" }}>
            {validationErrors.map((err, i) => {
              const hasTarget = Boolean(err.headerField || err.sectionId != null || err.lineId);
              return (
                <div
                  key={i}
                  onClick={() => goToError(err)}
                  style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", borderTop: "1px solid #fecaca", cursor: hasTarget ? "pointer" : "default", fontSize: 13 }}
                >
                  <span style={{ fontWeight: 700, color: "#b91c1c", width: 20, flexShrink: 0 }}>{i + 1}.</span>
                  <span style={{ color: "#b91c1c", flex: 1 }}>{err.message}</span>
                  {hasTarget && <ChevronRight size={16} style={{ color: "#b91c1c", flexShrink: 0 }} />}
                </div>
              );
            })}
          </div>
      ...
      <div style={{ marginBottom: 16, overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
          <table ref={tableRef} style={{ width: "100%", minWidth: 700, borderCollapse: "collapse", tableLayout: "fixed" }}>
            <colgroup>
              <col style={{ width: 36 }} />
            ...
            <tbody>
              {data.groups.map((group: Group) => {
                ...
                <React.Fragment key={group.id}>
                  {/* Section header */}
                  {!isContinuation && <tr data-section-id={group.sectionId} data-section-type={group.sectionType || group.id} style={{ background: DARK_HEADER, cursor: "pointer", boxShadow: highlightSectionId === String(group.sectionId) ? "inset 0 0 0 3px #facc15" : "none", transition: "box-shadow 0.4s" }} onClick={() => toggleGroup(group.id)}>
                    {group.expanded ? (
                  ...
                  {group.expanded && (() => {
                    ...
                    const renderFinancialRow = (value: ValueRow) => {
                      ...
                      const isHighlighted = highlightLineId === value.id;
                      return (
                        <tr key={value.id} data-line-id={value.id} className="feval-calc-row" style={{ background: isHighlighted ? "#fef08a" : READONLY_ROW, boxShadow: isHighlighted ? "inset 0 0 0 2px #f59e0b" : "none", transition: "background 0.4s" }}>
                    ...
                    if (isContinuation || isServerDrivenSection(group)) {
                      return <>{visibleValues.map(renderFinancialRow)}</>;
                    }
                    ...
                    const renderEditable = (value: ValueRow, _idx: number) => {
                      ...
                      const isHighlighted = highlightLineId === value.id;
                      return (
                        <tr key={value.id} data-line-id={value.id} style={{ background: isHighlighted ? "#fef08a" : bg, boxShadow: isHighlighted ? "inset 0 0 0 2px #f59e0b" : "none", transition: "background 0.4s" }}>
                                ...
                                <input type="text"
                                  ...
                                  onFocus={() => {
                                    const displayed = val === 0 ? "" : rawRow
                                      ? String(parseFloat(val.toFixed(decimalsForScale(displayScale))))
                                      : String(parseFloat(((val * fxMultiplier) / SCALE_DIVISOR[displayScale]).toFixed(decimalsForScale(displayScale))));
                                  ...
                                  className="feval-line-value"
                                  style={{ width: "100%", textAlign: "right", border: "none", background: isReadonly ? "#f3f4f6" : "transparent", fontSize: 12, color: val < 0 ? "#ff1f3d" : "#1f2937", outline: "none", height: 30, cursor: isReadonly ? "default" : "text" }}
                                />
                              </td>
                              );
                            })}
                    ...
                    const renderCalcRow = (value: ValueRow) => (
                      <tr key={value.id} data-line-id={value.id} className="feval-calc-row" style={{ background: highlightLineId === value.id ? "#fef08a" : READONLY_ROW, boxShadow: highlightLineId === value.id ? "inset 0 0 0 2px #f59e0b" : "none", transition: "background 0.4s" }}>
                    ...
                    return (<>
                      ...
                      {!isReadonly && group.isNewLineRequired !== "N" && (
                        <tr key="add-btn"><td colSpan={3 + numYears + 1} style={{ padding: "8px 14px", borderBottom: "1px solid #e5e7eb" }}><button type="button" onClick={() => addValueRow(group.id)} style={{ fontSize: 12, background: "none", border: "none", cursor: "pointer", color: BRAND, fontWeight: 600, display: "flex", alignItems: "center", gap: 4, height: 18 }}>+ Add {addLineLabel} Line</button></td></tr>
                      )}
                      {hasOngoing ? (<>
